...
# ## Copy of the 'killnet' generator script.
function copyApp() {
...
# ## Download and populate the killnet node for the first time.
function ipsetUpd() {
...
if [ -f "${rulesLocation}/killnet.ip" ]; then
 ...
 # ## Check if the Spamhaus IP list file exists.
 if [ ! -f "${rulesLocation}"/killnet.ip ]; then
...
function ipsetCron() {
 # ## Copying the cron file for KillNet DDoSllNet DDoS.
...
# ## Example of using the killnet-nodes node.
